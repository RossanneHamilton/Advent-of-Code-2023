    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        for (let j = 0; j < arr[i].length; j++) {
            for (let k = 0; k < arr[i][j].length; k++) {
                if ((arr[i][j][k].includes("red")) && (Number(arr[i][j][k].match(/\d+/g)[0]) > red)) {                    
                    red = Number(arr[i][j][k].match(/\d+/g)[0]);
                }
                if ((arr[i][j][k].includes("blue")) && (Number(arr[i][j][k].match(/\d+/g)[0]) > blue)) {
                    blue = Number(arr[i][j][k].match(/\d+/g)[0]);
                }
                if ((arr[i][j][k].includes("green")) && (Number(arr[i][j][k].match(/\d+/g)[0]) > green)) {
...
function powerArray(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        newArr.push(power); // Push the sums of colors to ballTotalArray
    }
    return newArr;
}
...
function GetIt(arr)
{
  ...
  for(var i=0;i<arr.length;i++)
    {
     console.log("chack arr " + arr[i])
      for(var j=0;j<arr[i].length;j++)
        {
          console.log(arr[i])
        }
      gamePow.push(arr[i][0]*arr[i][1]*arr[i][2]);
    }
  let result =0;
  for(var j=0;j<gamePow.length;j++)
    {
      if(gamePow[j]>0)
          result+=gamePow[j]; 
    }
...
console.log(powerArray(dataArr))

console.log(GetIt(dataArr));
