            ...
            if(lineArr[i][j]!='.')
              {
                ...
                let curr = lineArr[i][j];
                 console.log("current "+curr);
                let nan = isNaN(curr);
                let checkNext=0;
                //check if there is adjacent number/symbol
                //check -1 +1 of same row
                //check -1 1 +1 of next row
                if(i>0)
                  {
                    checkNext = lineArr[i-1][j];
                    if(nan!=isNaN(checkNext))
                      {
                          PulloutNumber(i,j,nan, i-1,j);
                      }
                    if(j+1<lineArr[i].length)
                   { checkNext = lineArr[i-1][j+1];
                     if(nan!=isNaN(checkNext))
                      {
                          PulloutNumber(i,j,nan, i-1,j+1);
                      }
                   }
                    if(j>0)
                      {
                ...
                if(j-1>=0)
                  {
                    ...
                    //check prev of same row
                    if(lineArr[i][j-1]!='.')//&&curr!='.')
                      {
                        checkNext = lineArr[i][j-1];
                       if(nan==!isNaN(checkNext))
                       {
                       //  console.log(curr+ " passed next to: "+checkNext);
                       // console.log("checking previous on same line");
                         PulloutNumber(i,j,nan, i,j-1);
                         
                       }
                      }
                   
                    if(i+1<=lineArr.length-1)
                      {
                        //check prev of next row
                        if(lineArr[i+1][j-1]!='.')
                          { checkNext = lineArr[i+1][j-1];
                       ...
                       {
                       //  console.log(curr+ " passed next to: "+checkNext);
                       //  console.log("checking previous on next line");
                          PulloutNumber(i,j,nan, i+1,j-1);
                       }
                           else
                             {
                               // PulloutNumber(i+1,j-1,nan, i,j);
                             }
                           
                            // console.log(lineArr[i+1][j-1]+ " passed pos next to: "+lineArr[i+1][j-1]);
                          }
                      }
                  }
                //check next of same row and same of next row
                 if(i+1<=lineArr.length-1)
                  {
                   
                    //check same of next row
                    if(lineArr[i+1][j]!='.')
                      {
                        checkNext = lineArr[i+1][j];
                         if(nan==!isNaN(checkNext))
                       {
                       //  console.log(curr+ " passed next to: "+checkNext);
                       //  console.log("checking same position on next line");
                          PulloutNumber(i,j,nan, i+1,j);
                       }
                       // console.log(lineArr[i+1][j]+ " passed next to: "+lineArr[i+1][j]);
                      }
                  }
                if(j+1<=lineArr[i].length-1)
                  {
                    //check check next of same row
                     if(lineArr[i][j+1]!='.')
                      {
                        checkNext = lineArr[i][j+1];
                         if(nan==!isNaN(checkNext))
                       {
                       //  console.log(curr+ " passed next to: "+checkNext);
                       //  console.log("checking next on same line");
                         PulloutNumber(i,j,nan, i,j+1);
                       }
                     
                      }
                    //check next of next row
                    if(i<lineArr.length-1)
                         ...
                         if(lineArr[i+1][j+1]!='.')
                      {
                       ...
                       {
                       //  console.log(curr+ " passed next to: "+checkNext);
                       //  console.log("checking next on next line");
                          PulloutNumber(i,j,nan, i+1,j+1);
                       }
                       // console.log(lineArr[i][j]+ " passed next to :"+ lineArr[i+1][j+1]);
                      }
                      }
                 }
                tempArr.push(lineArr[i][j]+"_"+j);
              }
          }
        nArray.push(tempArr);
      }
...
GetPosition();
GetTotal();
function PulloutNumber(_i,_j,_isNan,_ii,_jj)
{
  let temp = "";
          ...
          for(var i=-2;i<3;i++)
            {
              ...
              if(i+_j>=0&&i+_j<lineArr.length)
                {
                 
                  tempCheck += lineArr[_i][i+_j];
                }
           ...
           for(var i=-2;i<3;i++)
            {
              ...
              if(i+_jj>=0&&i+_jj<lineArr.length)
                {
                  tempCheck+= lineArr[_ii][i+_jj];
                }
